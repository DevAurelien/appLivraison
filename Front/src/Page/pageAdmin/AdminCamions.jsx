  ...
  return (
    <div className="w-full h-full flex flex-col gap-4 items-center p-4">
          ...
          <div className="flex gap-2 text-[0.8rem] py-1 border-b border-(--border-default)">
            {salaries.map((sal, index) => (
              <p className="flex gap-2" key={index}>
                {sal.driver ? <DriverIcone className="size-4 flex" /> : null}
                {sal.driver}
                {sal.rippeur}{" "}
                {sal.rippeur ? <RippeurIcone className="size-4 flex" /> : null}
              </p>
            ))}
          </div>
      ...
      <div className="relative flex justify-center">
        <button
          type="submit"
          className="z-5 flex gap-2 cursor-pointer items-center justify-center rounded-xl bg-yellow-300 px-4 py-2 text-black disabled:cursor-not-allowed"
        >
         <PlusIcone className="size-6 p-1" color1="black"/> Ajouter un camion
        </button>
      </div>
    </div>
  );
